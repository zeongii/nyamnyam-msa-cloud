    ...
    stages {
        ...
        stage('Build Docker Images') {
            steps {
                script {
                    dir('nyamnyam.kr') {
                        sh "cd server/config-server && docker build -t ${DOCKER_CREDENTIALS_ID}/nyamnyam-config-server:1.1 ."
                    }
            ...
            steps {
                script {
                    ...
                    servicesList.each { service ->
                        ...
                        sh "docker push ${DOCKER_CREDENTIALS_ID}/nyamnyam-${serviceName}:1.1"
                    }
                }
            }
        }
        ...
        stage('Cleaning up') {
            steps {
                script {
                    ...
                    servicesList.each { service ->
                        def serviceName = service.split('/')[1] // 서비스 이름 추출
                        sh "docker rmi ${DOCKER_CREDENTIALS_ID}/nyamnyam-${serviceName}:1.1" // Clean up the pushed image
                    }
                }
            }
        }
